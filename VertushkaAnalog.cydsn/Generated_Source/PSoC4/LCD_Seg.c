...
static void LCD_Seg_WriteFrameBuffer(reg32 regAddr[], uint32 bitNumber, uint32 state);
static uint32 LCD_Seg_ReadFrameBuffer(const reg32 regAddr[], uint32 bitNumber);

/* This section contains look-up tables for different kinds of displays. */
#ifdef LCD_Seg_7SEG
    static const uint8 LCD_Seg_7SegDigits[] = {
      /*  '0'    '1'    '2'    '3'    '4'    '5'    '6'    '7' */
...
#ifdef LCD_Seg_14SEG
    static const uint16 LCD_Seg_14SegChars[] = {
    /*------------------------------------------------------------*/
    /*                           Blank                            */
    /*------------------------------------------------------------*/
...
#ifdef LCD_Seg_16SEG
    static const uint16 LCD_Seg_16SegChars[] = {
    /*------------------------------------------------------------*/
    /*                           Blank                            */
    /*------------------------------------------------------------*/
    0x0000u,0x0000u,0x0000u,0x0000u,0x0000u,0x0000u,0x0000u,0x0000u,
    ...
    static const uint8 LCD_Seg_charDotMatrix[][LCD_Seg_DM_CHAR_WIDTH] = {
...
static const uint32 LCD_Seg_dividersLS[][LCD_Seg_CONTRAST_DIVIDERS_NUMBER] = {
{0xD5u, 0xD5u, 0xD5u, 0xD5u, 0xD5u, 0xD5u, 0xA0u, 0x6Bu, 0x35u, 0x00u},
{0x13u, 0x13u, 0x13u, 0x13u, 0x13u, 0x13u, 0x17u, 0x1Au, 0x1Du, 0x20u}};
static const uint32 LCD_Seg_dividersHS[][LCD_Seg_CONTRAST_DIVIDERS_NUMBER] = {
{0x0001u, 0x0001u, 0x0001u, 0x0001u, 0x0000u, 0x0000u, 0x0000u, 0x0000u, 0x0000u, 0x0000u},
{0x0000u, 0x0000u, 0x0000u, 0x0000u, 0x0000u, 0xFFFFu, 0xFFFFu, 0xFFFFu, 0xFFFFu, 0xFFFFu}};

uint32 LCD_Seg_initVar = 0u;
static uint32 LCD_Seg_modeState = LCD_Seg_LCD_MODE;
static uint32 LCD_Seg_invertState = LCD_Seg_STATE_NORMAL;
static uint32 LCD_Seg_contrastState = LCD_Seg_CONTRAST;
static uint32 LCD_Seg_commons[] = {LCD_Seg_Com0, LCD_Seg_Com1, LCD_Seg_Com2, LCD_Seg_Com3};


/*******************************************************************************
* Function Name: LCD_Seg_WriteFrameBuffer
********************************************************************************
*
* Summary:
...
static void LCD_Seg_WriteFrameBuffer(reg32 regAddr[], uint32 bitNumber, uint32 state)
{
    ...
    row  = LCD_Seg_EXTRACT_ROW(bitNumber) % LCD_Seg_MAX_BUFF_ROWS;
    port = LCD_Seg_EXTRACT_PORT(bitNumber) + ((LCD_Seg_EXTRACT_ROW(bitNumber) /
           LCD_Seg_MAX_BUFF_ROWS) * LCD_Seg_DATA_REGS_OFFSET);
    pin  = LCD_Seg_EXTRACT_PIN(bitNumber) * LCD_Seg_MAX_BUFF_ROWS;

    /* Write new bit value to the frame buffer. */
    mask = (uint32)(~((uint32)(LCD_Seg_PIXEL_STATE_ON << (pin + row))));

    regAddr[port] = (regAddr[port] & mask) | (uint32)((state & LCD_Seg_PIXEL_STATE_ON) << (pin + row));
}


/*******************************************************************************
* Function Name: LCD_Seg_ReadFrameBuffer
********************************************************************************
*
* Summary:
...
static uint32 LCD_Seg_ReadFrameBuffer(const reg32 regAddr[], uint32 bitNumber)
{
    ...
    pixelState = (uint32)(regAddr[port] >> (pin + row)) & LCD_Seg_PIXEL_STATE_ON;
...
void LCD_Seg_Init(void)
{
    ...
    LCD_Seg_DIVIDER_REG = LCD_Seg_INIT_DIVIDERS;

    /* Set Configuration Register */
    LCD_Seg_CONTROL_REG = LCD_Seg_INIT_CONTROL;

    /* Need to clear display to start normal work.
    *  LCD_Seg_ClearDisplay() function initializes Commons in the frame buffer as well.
    */
...
*
* Global variables:
*  LCD_Seg_modeState - holds the current LCD speed mode (LS/HS).
*
*******************************************************************************/
void LCD_Seg_Enable(void)
{
    if (LCD_Seg_SPEED_LS == LCD_Seg_modeState)
    {
        LCD_Seg_CONTROL_REG = (LCD_Seg_CONTROL_REG & ((uint32)(~LCD_Seg_ENABLE_MASK)))
                                       | LCD_Seg_LS_EN;
    }
    else /* (LCD_Seg_SPEED_HS == LCD_Seg_modeState) */
    {
        LCD_Seg_CONTROL_REG = (LCD_Seg_CONTROL_REG & ((uint32)(~LCD_Seg_ENABLE_MASK)))
                                       | LCD_Seg_HS_EN;
    }
...
{
    LCD_Seg_CONTROL_REG &= (uint32)(~LCD_Seg_ENABLE_MASK);
}


/*******************************************************************************
* Function Name: LCD_Seg_SetSpeedMode
********************************************************************************
*
* Summary:
...
* Global variables:
*  LCD_Seg_modeState - holds the current LCD speed mode (LS/HS).
*  LCD_Seg_contrastState - holds the current LCD contrast.
*
*******************************************************************************/
void LCD_Seg_SetSpeedMode(uint32 mode)
{
    uint32 contrast = (LCD_Seg_contrastState / 10u) - 1u;

    if (LCD_Seg_modeState != mode)
    {
        LCD_Seg_CONTROL_REG = (LCD_Seg_CONTROL_REG & ((uint32)(~LCD_Seg_MODE)))
                                       | (((uint32)(mode  << LCD_Seg_MODE_SHIFT)) & LCD_Seg_MODE);

        if (LCD_Seg_SPEED_LS == mode)
        {
            /* Set sub-frame and dead time dividers */
            LCD_Seg_DIVIDER_REG = (LCD_Seg_DIVIDER_REG & ((uint32)(~LCD_Seg_DIVIDER_MASK)))
                            | LCD_Seg_dividersLS[LCD_Seg_SUBFR_DIVS][contrast]
                            | ((uint32)((uint32)LCD_Seg_dividersLS[LCD_Seg_DEAD_DIVS][contrast] <<
                                                LCD_Seg_DEAD_DIV_MASK_SHIFT));

            LCD_Seg_modeState = LCD_Seg_SPEED_LS;
        }
        else /* (LCD_Seg_SPEED_HS == mode) */
        {
            /* Set sub-frame and dead time dividers */
            LCD_Seg_DIVIDER_REG = (LCD_Seg_DIVIDER_REG & ((uint32)(~LCD_Seg_DIVIDER_MASK)))
                            | LCD_Seg_dividersHS[LCD_Seg_SUBFR_DIVS][contrast]
                            | ((uint32)((uint32)LCD_Seg_dividersHS[LCD_Seg_DEAD_DIVS][contrast] <<
                                                LCD_Seg_DEAD_DIV_MASK_SHIFT));

            LCD_Seg_modeState = LCD_Seg_SPEED_HS;
        }
    }
}
...
/*******************************************************************************
* Function Name: LCD_Seg_SetOperationMode
********************************************************************************
*
* Summary:
...
{
    LCD_Seg_CONTROL_REG = (LCD_Seg_CONTROL_REG & ((uint32)(~LCD_Seg_OP_MODE)))
                                   | (((uint32)(mode << LCD_Seg_OP_MODE_SHIFT)) & LCD_Seg_OP_MODE);
}


/*******************************************************************************
* Function Name: LCD_Seg_SetBiasType
********************************************************************************
*
* Summary:
...
{
    LCD_Seg_CONTROL_REG = (LCD_Seg_CONTROL_REG & ((uint32)(~LCD_Seg_BIAS_MASK)))
                                 | (((uint32)(bias << LCD_Seg_BIAS_MASK_SHIFT)) & LCD_Seg_BIAS_MASK);
}


/*******************************************************************************
* Function Name: LCD_Seg_SetWaveformType
********************************************************************************
*
* Summary:
...
{
    LCD_Seg_CONTROL_REG = (LCD_Seg_CONTROL_REG & ((uint32)(~LCD_Seg_TYPE)))
                                   | (((uint32)(type << LCD_Seg_TYPE_SHIFT)) & LCD_Seg_TYPE);
}


/*******************************************************************************
* Function Name: LCD_Seg_SetContrast
********************************************************************************
*
* Summary:
...
uint32 LCD_Seg_SetContrast(uint32 contrast)
{
    ...
    if ((LCD_Seg_MIN_CONTRAST <= contrast) && (contrast <= LCD_Seg_MAX_CONTRAST))
    {
        contrastIndex = (contrast / 10u) - 1u;
        if (LCD_Seg_SPEED_LS == LCD_Seg_modeState)
        {
            if (LCD_Seg_dividersLS[LCD_Seg_DEAD_DIVS][contrastIndex] !=
                LCD_Seg_dividersLS[LCD_Seg_DEAD_DIVS][contrastIndex + 1u])
            {
                /* Set sub-frame and dead time dividers */
                LCD_Seg_DIVIDER_REG = (LCD_Seg_DIVIDER_REG
                        & ((uint32)(~LCD_Seg_DIVIDER_MASK)))
                        | LCD_Seg_dividersLS[LCD_Seg_SUBFR_DIVS][contrastIndex]
                        | ((uint32)((uint32)LCD_Seg_dividersLS[LCD_Seg_DEAD_DIVS][contrastIndex] <<
                                            LCD_Seg_DEAD_DIV_MASK_SHIFT));
        ...
        {
            if (LCD_Seg_dividersHS[LCD_Seg_DEAD_DIVS][contrastIndex] !=
                LCD_Seg_dividersHS[LCD_Seg_DEAD_DIVS][contrastIndex + 1u])
            {
                /* Set sub-frame and dead time dividers */
                LCD_Seg_DIVIDER_REG = (LCD_Seg_DIVIDER_REG
                        & ((uint32)(~LCD_Seg_DIVIDER_MASK)))
                        | LCD_Seg_dividersHS[LCD_Seg_SUBFR_DIVS][contrastIndex]
                        | ((uint32)((uint32)LCD_Seg_dividersHS[LCD_Seg_DEAD_DIVS][contrastIndex] <<
                                            LCD_Seg_DEAD_DIV_MASK_SHIFT));
                status = CYRET_SUCCESS;
            }
        }

        LCD_Seg_contrastState = contrast; /* Saves new contrast value */
    }
...
*
* Global variables:
*  LCD_Seg_invertState - holds the current LCD state (Inverted display
...
void LCD_Seg_WriteInvertState(uint32 invertState)
{
    ...
    if (invertState != LCD_Seg_invertState)
    {
        /* Invert entire frame buffer */
        LCD_Seg_DATA0_REG = (uint32)(~LCD_Seg_DATA0_REG);
        LCD_Seg_DATA1_REG = (uint32)(~LCD_Seg_DATA1_REG);
        LCD_Seg_DATA2_REG = (uint32)(~LCD_Seg_DATA2_REG);
        LCD_Seg_DATA3_REG = (uint32)(~LCD_Seg_DATA3_REG);
        LCD_Seg_DATA4_REG = (uint32)(~LCD_Seg_DATA4_REG);

        /* Reinitialize the commons */
        for (i = 0u; i < LCD_Seg_COM_NUM; i++)
        {
            /* Clear commons pin data after frame buffer inverting */
            for (j = 0u; j < LCD_Seg_COM_NUM; j++)
            {
                LCD_Seg_WriteFrameBuffer(LCD_Seg_DATA0_PTR,
                ((LCD_Seg_commons[i] & ((uint32)(~LCD_Seg_ROW_MASK))) |
                ((uint32)(j << LCD_Seg_ROW_SHIFT))), LCD_Seg_PIXEL_STATE_OFF);
            }

            /* Set pin data for commons */
            LCD_Seg_WriteFrameBuffer(LCD_Seg_DATA0_PTR, LCD_Seg_commons[i],
                                              LCD_Seg_PIXEL_STATE_ON);
        }

        LCD_Seg_invertState = invertState;
    }
}
...
void LCD_Seg_ClearDisplay(void)
{
    ...
    LCD_Seg_DATA0_REG = 0Lu;
    LCD_Seg_DATA1_REG = 0Lu;
    LCD_Seg_DATA2_REG = 0Lu;
    LCD_Seg_DATA3_REG = 0Lu;
    LCD_Seg_DATA4_REG = 0Lu;
    ...
    * display.
    */
    if (LCD_Seg_invertState == LCD_Seg_STATE_INVERTED)
    {
        LCD_Seg_WriteInvertState(LCD_Seg_STATE_INVERTED);
    }
}
...
uint32 LCD_Seg_WritePixel(uint32 pixelNumber, uint32 pixelState)
{
    ...
    {
        if (LCD_Seg_PIXEL_STATE_INVERT == pixelState)
        {
            /* Invert actual pixel state */
            pixelState = LCD_Seg_ReadPixel(pixelNumber);
            pixelState = ((uint32)(~pixelState)) & LCD_Seg_PIXEL_STATE_ON;
        }

        /* Write new pixel's value to the frame buffer. */
        LCD_Seg_WriteFrameBuffer(LCD_Seg_DATA0_PTR, pixelNumber, pixelState);
...
*     LCD_Seg_PIXEL_STATE_ON        Pixel on.
*     LCD_Seg_PIXEL_STATE_OFF       Pixel off.
*     LCD_Seg_PIXEL_UNKNOWN_STATE   Not assigned pixel.
*
*******************************************************************************/
uint32 LCD_Seg_ReadPixel(uint32 pixelNumber)
{
    uint32 pixelState = LCD_Seg_PIXEL_UNKNOWN_STATE;

    if (LCD_Seg_NOT_CON != pixelNumber)
    {
        pixelState = LCD_Seg_ReadFrameBuffer(LCD_Seg_DATA0_PTR, pixelNumber);
    }
...
{
    static const uint32 LCD_Seg_disp0[2u][7u] = {{LCD_Seg_NOT_CON, LCD_Seg_NOT_CON, LCD_Seg_NOT_CON, LCD_Seg_NOT_CON,
    LCD_Seg_NOT_CON, LCD_Seg_NOT_CON, LCD_Seg_NOT_CON}, {LCD_Seg_NOT_CON, LCD_Seg_NOT_CON, LCD_Seg_NOT_CON,
    ...
    if ((position / LCD_Seg_DIGIT_NUM_0) == 0u)
    {
        ...
        {
            (void)LCD_Seg_WritePixel(LCD_Seg_disp0[position][i],
            (((uint32)((uint32)LCD_Seg_7SegDigits[digit] >> i)) & LCD_Seg_PIXEL_STATE_ON));
        }
    }
}
